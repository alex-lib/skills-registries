...
**Централизованное хранилище скиллов (Centralized AI Skills Registry)** — это единая система хранения, версионирования и распространения переиспользуемых инструкций для AI-агентов (скиллов, а иногда и slash-команд), которая заменяет разрозненное хранение этих инструкций в личных заметках, чатах, локальных файлах или разных несинхронизированных git-репозиториях сотрудников.

Проще говоря — это то же самое, чем для кода является внутренний npm/PyPI-реестр компании: не каждый пишет свою версию одной и той же функции с нуля, а есть один источник правды, откуда все берут и куда все обновляют.

## Зачем оно нужно

Без централизованного хранилища в компании неизбежно возникает следующая картина (мы фактически наблюдали её в описании задач для SkillReg — "у нас скиллы разбросаны по Google Docs, git-репо, Notion"):

- Один и тот же полезный скилл (например, "ревью пул-реквеста по стандартам компании") **изобретается заново** в каждой команде, потому что никто не знает, что он уже есть у соседей.
- Хороший скилл **не обновляется** одновременно у всех — кто-то работает по устаревшей, а иногда и ошибочной версии.
- Никто не может ответить на вопрос **"кто и когда добавил этот скилл, и проверял ли его хоть кто-то на безопасность"** — а скилл это, по сути, инструкции, которые агент выполняет с доверием, то есть потенциальный вектор атаки (prompt injection, вредоносный код внутри).
- Нетехническим сотрудникам (не разработчикам) вообще негде и не с чем работать — если скиллы лежат только в git, доступ к ним фактически есть только у тех, кто знает, что такое git.

## Какие задачи оно решает

| Задача | Как решается |
|---|---|
| **Единый источник правды** | Один каталог для всей компании вместо разрозненных копий |
| **Версионирование** | Semver-подобная система версий — можно опубликовать V2, не сломав тех, кто ещё сидит на V1 |
| **Governance / контроль качества** | Review/approval-флоу — скилл не появляется у всех сотрудников сразу после того, как кто-то его написал, а проходит проверку |
| **Безопасность** | Автоматическое сканирование скиллов на вредоносный код/секреты перед публикацией (антивирус, статический анализ) |
| **Аудит и ответственность (attribution)** | Фиксация, кто опубликовал, кто одобрил, кто установил — важно для расследования инцидентов и комплаенса |
| **Отзыв доступа (revocation)** | Если в скилле нашли уязвимость или сотрудник уволился — можно мгновенно отозвать доступ, а не полагаться на то, что все вручную удалят файл |
| **Единообразная установка/обновление** | Одна команда (`pull`, `install`, `bundle`) вместо ручного копирования файлов каждым сотрудником |
| **Изоляция по командам (namespace)** | Скилл отдела продаж не путается со скиллом разработки, но при этом оба доступны из одного места |

## Почему им удобно пользоваться

**Для сотрудника (потребителя):**
- Не нужно знать, где физически хранится скилл — просто ищешь по названию в каталоге/поиске и ставишь одной командой.
- Обновление до актуальной версии — тоже одна команда, а не "спроси в чате, есть ли что-то новое".
- Понятно, какой скилл **одобрен** компанией к использованию, а какой ещё на модерации — не нужно гадать, можно ли доверять содержимому.

**Для автора скилла:**
- Публикация версии — не "разошли всем в Slack файл", а формальный, отслеживаемый процесс с историей изменений.
- Видно количество установок/использования — обратная связь, какие скиллы реально ценны команде.

**Для компании в целом:**
- Единая точка контроля рисков (security-скан, approval) вместо того, чтобы полагаться на добросовестность каждого отдельного сотрудника.
- Возможность в любой момент ответить на вопрос "что из AI-инструкций сейчас реально используется в компании и кем" — то, что без централизации в принципе невозможно узнать.
- Экономия времени: скилл, написанный один раз одной командой, автоматически доступен всем остальным, а не переизобретается параллельно в трёх разных отделах.
