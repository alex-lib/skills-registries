...
## 4. Базовая архитектура: GitLab/GitHub + `npx skills`
...
### 4.2 Где физически хранится skill
...
### 4.4 Как installer понимает, куда ставить skill
...
### 4.5 Как installer находит skills внутри repository
...
### 4.7 Может ли он устанавливать несколько библиотек параллельно
...
### 4.8 Как обновляется skill у пользователя
...
### 4.9 Workflow разработчика skill
...
### 4.10 Что будет при 1000 skills
...
### 4.11 Плюсы и минусы GitLab/GitHub + `npx skills`
...
## 5. RoleCraft - альтернативный installer поверх GitLab/GitHub
...
## 6. Skilly — self-hosted корпоративное хранилище
...
## 7. SkillReg — готовый SaaS private registry
...
## 8. Сравнение по признаку использования в компании
...
Что меня лично зацепило из плюсов и минусов:

### GitLab/GitHub + `npx skills`
1. +- Сложно для пользователя который не знаком с git и работой в терминале
2. - При получении списка скиллов репозитория ты сразу получаешь например 1000 доступных что замедляет выбор
3. + Быстрая загрузка и выгрузка большого количества скиллов

### GitLab/GitHub + Rolecraft
Мало чем отличается от GitLab/GitHub + `npx skills` заисключнием наличия команды `bundle`

### Skilly
1. + Приятный UI
2. + Возможность обсуждение скилов
3. + Система рейтинга скиллов
4. +- Без работы через терминал не обойтись - но минимально
5. - Медленная и костыльная загрузка большого количества скиллов
6. +- Нужно разворачивать руками на сервере (+ minio, caddy, postgres и прочая настройка), но зато полный контроль

### Skillreg
1. + Наличие десктоп риложения
2. + UI не "загруженный" и интуитивно понятный без гайда
3. - Без платной подписики смысла использовать нет
4. + Большое количество скиллов подгружается со средней скоростью по сравнению с GitLab/GitHub + `npx skills` и Skilly
5. + Возможность создавать свои
6. + Сложность настройки = нулевая 
---
